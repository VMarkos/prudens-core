...
class Variable: # TODO Consider adding fields about the (rule), literal and position of the variable.
    ...
    def from_dict(cls, init_dict: Dict) -> Variable:
        ...
        variable.original_string = utils.parse_dict_prop(init_dict, "original_string", "Variable", default_value = "", expected_types = [str])
        variable.name = utils.parse_dict_prop(init_dict, "name", "Variable", expected_types = [str])
        variable.code = utils.parse_dict_prop(init_dict, "code", "Variable", default_value = None, expected_types = [type(None), CodeType])
        try:
        ...
        try:
            ...
            raise KeyError(f"Wrong variable type provided: '{var_type}'. Accepted types: {[x.name for x in VariableType]}.")
        return variable
