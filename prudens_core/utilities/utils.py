...
def parse_dict_prop(init_dict: Dict,
                    dict_key: str,
                    class_name: str,
                    **kwargs: Dict) -> None:
    try:
        ...
        if "default_value" in kwargs.keys():
            ...
            raise KeyError(f"Missing key '{dict_key}' in {class_name} initialization from dict.")
    if "expected_types" in kwargs.keys():
        if type(object_attr) not in kwargs["expected_types"]:
            ...
            raise TypeError(f"Expected input of type {types_str} for {class_name}.{dict_key} but received {type(object_attr)}.")
    return object_attr