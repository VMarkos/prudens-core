...
class PriorityRelation:
    __slots__ = ("original_string", "rule_heads", "rule_indices", "indice_rules", "priorities",
                 "conflict_matrix", "default", "candidate_conflicts")
    ...
    def __init__(self, priority_str: str, rules: Dict[str, Rule]) -> None:
        ...
        self.candidate_conflicts: Dict[str, Set[str]] = { rn_1: {rn_2 for rn_2, r_2 in rules.items()\
                                                                  if r_1.head.name == r_2.head.name\
                                                                     and r_1.head.sign != r_2.head.sign}\
    ...
    def from_dict(cls, init_dict) -> PriorityRelation:
        pr = cls.__new__(cls)
        pr.original_string = utils.parse_dict_prop(init_dict, "original_string", "PriorityRelation", default_value = "", expected_types = [str])
        pr.default = utils.parse_dict_prop(init_dict, "default", "PriorityRelation", expected_types = [bool])
        try:
            rule_heads = init_dict["rule_heads"]
        except KeyError:
            raise KeyError(f"Missing key 'rule_heads' in PriorityRelation initialization from dict.")
        if type(rule_heads) != dict:
            raise TypeError(f"Expected input of type 'dict' for PriorityRelation.rule_heads but received {type(rule_heads)}.")
        pr.rule_heads = dict()
        for rn, head in rule_heads.items():
            if type(rn) != str:
                raise TypeError(f"Expected input of type 'str' for PriorityRelation.rule_heads.key but received {type(rn)}.")
            try:
                pr.rule_heads[rn] = Literal.from_dict(head)
            except KeyError as e:
                raise KeyError(f"While parsing priority relation from a dict, literal dict {head} in provided rule_heads "
                               "could not be properly parsed to a literal.") from e
            except TypeError as e:
                raise TypeError(f"While parsing priority relation from a dict, literal dict {head} in provided rule_heads "
                               "could not be properly parsed to a literal.") from e
        ...
        try:
            ...
            raise KeyError(f"Missing key 'rule_indices' in PriorityRelation initialization from dict.")
        if type(rule_indices) != dict:
            raise TypeError(f"Expected input of type 'dict' for PriorityRelation.rule_indices but received {type(rule_indices)}.")
        pr.rule_indices = dict()
        for k, v in rule_indices.items():
            if type(k) != str:
                raise TypeError(f"Expected input of type 'str' for PriorityRelation.rule_indices.keys but received {type(k)}.")
            if type(v) != int:
                raise TypeError(f"Expected input of type 'int' for PriorityRelation.rule_indices.values but received {type(v)}.")
            pr.rule_indices[k] = v
        pr.indice_rules = { v: k for k, v in pr.rule_indices.items() }
        try:
            priorities = init_dict["priorities"]
        except KeyError:
            raise KeyError(f"Missing key 'priorities' in PriorityRelation initialization from dict.")
        if type(priorities) != list:
            raise TypeError(f"Expected input of type 'list' for PriorityRelation.priorities but received {type(priorities)}.")
        pr.priorities = set()
        for p in priorities:
            if len(p) != 2:
                raise ValueError(f"Priority contains {len(p)} values. Expected 2.")
            if type(p) != list:
                raise TypeError(f"Expected input of type 'list' for PriorityRelation.priorities.priority but received {type(p)}.")
            if type(p[0]) != int:
                raise TypeError(f"Expected input of type 'int' in priority {p} as a first element but received {type(p[0])}.")
            if type(p[1]) != int:
                raise TypeError(f"Expected input of type 'int' in priority {p} as a second element but received {type(p[1])}.")
    ...
    def is_prior(self, rule_1: str, rules: Dict[str, Set[Substitution]], main_sub: Substitution) -> bool:
        ...
        is_prior: bool = True
        for rule_2 in self.candidate_conflicts[rule_1].intersection(rules.keys()): # This is better than using `filter()`.
            ...
            if (ind_1, ind_2) not in self.priorities and (ind_2, ind_1) not in  self.priorities:
