...
from prudens_core.errors.RuntimeErrors import (
    VariableNotFoundInSubstitutionError,
    DuplicateValueError,
)
...
class Substitution:
    ...
    def __init__(self, other: Union[None, Substitution] = None) -> None:
        ...
        if other == None:
            self.sub: Dict[Variable, Constant] = (
                dict()
            )  # FIXME Why is this both Variable and Constant?
    ...
    def from_dict(cls, init_dict) -> Variable:
        ...
        try:
            ...
            raise KeyError(
                f"Missing key 'sub' in Substitution initialization from dict."
            )
        if type(sub_dict) != dict:
            raise TypeError(
                f"Expected input of type 'dict' for Substitution.sub but received {type(sub_dict)}."
            )
        sub.sub = dict()
        for v, c in sub_dict.items():
            if type(v) != str:
                raise TypeError(
                    f"Expected input of type 'str' for Substitution.sub.key but received {type(v)}."
                )
            try:
                variable = Variable(v)
            except PrudensSyntaxError as e:
                raise SyntaxError(
                    f"While parsing substitution from a dict, variable {v} could not be properly parsed."
                ) from e
            try:
                constant = Constant.from_dict(c)
            except KeyError as e:
                raise KeyError(
                    f"While parsing substitution from a dict, constant {c} could not be properly parsed."
                ) from e
        ...
        try:
            ...
            raise KeyError(
                f"Missing key 'equivalent_variables' in Substitution initialization from dict."
            )
        if type(ev_dict) != dict:
            raise TypeError(
                f"Expected input of type 'dict' for Substitution.equivalent_variables but received {type(ev_dict)}."
            )
        sub.equivalent_variables = dict()
        for v, vs in ev_dict.items():
            if type(v) != str:
                raise TypeError(
                    f"Expected input of type 'str' for Substitution.equivalent_variables.key but received {type(v)}."
                )
            try:
                variable = Variable(v)
            except PrudensSyntaxError as e:
    ...
    def to_dict(self) -> Dict:
        return {
            "sub": {str(k): v.to_dict() for k, v in self.sub.items()},
            "equivalent_variables": {
                str(k): [v.to_dict() for v in vs] for k, vs in self.equivalent_variables
            },
        }
    ...
    def __eq__(self, other: Substitution) -> bool:
        ...
        return (
            self.sub == other.sub
            and self.equivalent_variables == other.equivalent_variables
        )
