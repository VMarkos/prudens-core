...
class LiteralParser:
    ...
    def parse(self) -> ParsedLiteral:
        ...
        is_propositional: bool = re.fullmatch(r'-?(\?|!)?[a-z]\w*', self.literal_string, flags = re.ASCII)
        if is_propositional:
            sign: bool = self.literal_string[0] != "-"
            is_external: bool = sign and self.literal_string[0] == "?" or not sign and self.literal_string[1] == "?"
            is_action: bool = sign and self.literal_string[0] == "!" or not sign and self.literal_string[1] == "!"
            name: str = self.literal_string if sign else self.literal_string[1:]
            return ParsedLiteral(name, [], sign, 0, is_external, is_action)
        is_fol: bool = re.fullmatch(r'-?(\?|!)?[a-z]\w*\s*\(.+\)', self.literal_string, flags = re.ASCII)
        if is_fol:
            sign: bool = self.literal_string[0] != "-"
            is_external: bool = sign and self.literal_string[0] == "?" or not sign and self.literal_string[1] == "?"
            is_action: bool = sign and self.literal_string[0] == "!" or not sign and self.literal_string[1] == "!"
            ...
            for arg in arguments_string.split(","):
                ...
                if re.match(r'[a-z0-9]', arg[0]): # FIXME This ignores cases such as `1 + X`.
                    try:
            ...
            return ParsedLiteral(name, predicate_arguments, sign, len(predicate_arguments), is_external, is_action)
