    ...
    def parse(self):  # FIXME Reconsider whether the .+ solution to ignore sta
        ...
        if not re.match(rule_preamble_regex, self.rule_string):
            raise InvalidRuleNameError(
                self.rule_string
            )  # TODO There should be some way to prohibit some reserved words from being used.
        ...
        body_split: [str] = self.__rule_body_split(
            main_split[0]
        )  # FIXME This needs to be specified!
        if (
            not body_split
            or len(body_split) == 1
            and re.fullmatch(r"\s*", body_split[0])
        ):
        ...
        for literal_str in body_split:
            try:
                ...
                Exception
            ) as e:  # TODO Consider being more specific here, which means that you should somehow resolve the syntax + runtime error of having unassigned variables in math expressions.
                raise e
